  ...
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      className={cn(
        ...
        "max-sm:inset-x-0 max-sm:bottom-auto max-sm:top-[var(--vv-top,0px)] max-sm:flex max-sm:flex-col max-sm:h-[var(--vvh)] max-sm:max-h-none max-sm:w-full max-sm:max-w-none max-sm:translate-x-0 max-sm:translate-y-0 max-sm:rounded-none max-sm:border-0 max-sm:pb-[max(1rem,env(safe-area-inset-bottom))] max-sm:data-[state=closed]:zoom-out-100 max-sm:data-[state=open]:zoom-in-100 max-sm:data-[state=open]:slide-in-from-bottom-4",
        className,
      )}
      {...props}
    >
      {children}
      {/* Chip claro para o X ficar legível em qualquer fundo (inclusive heróis escuros). */}
      <DialogPrimitive.Close className="absolute right-3 top-3 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-background/80 text-foreground shadow-sm ring-1 ring-border backdrop-blur transition-colors hover:bg-background focus:outline-none focus:ring-2 focus:ring-ring">
        <X className="h-4 w-4" />
